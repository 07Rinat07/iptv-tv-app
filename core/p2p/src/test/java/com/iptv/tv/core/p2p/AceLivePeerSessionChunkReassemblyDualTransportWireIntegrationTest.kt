...
class AceLivePeerSessionChunkReassemblyDualTransportWireIntegrationTest {
    private val loopback: InetAddress = InetAddress.getByName("127.0.0.1")

    @Test
    fun `pool reassembles requested live chunks from verified uTP winner and advances cursor`() = runBlocking {
        DualLiveChunkPeer(loopback).use { peer ->
            ...
            val session = AceLivePeerSessionCoordinator(
                geometry = AceLiveTransportGeometry(
                    pieceLengthBytes = PIECE_DATA.size,
                    chunkLengthBytes = CHUNK_LENGTH_BYTES,
                    bitrate = 1
                ),
                    ...
                    events.filterIsInstance<AceLiveTcpPoolEvent.Ingress>().any { event ->
                        event.peerId == PEER_ID &&
                            event.result.metadataUpdates.any { window ->
                                window.minPiece == PIECE_NUMBER && window.maxPiece == 12L
                            }
                    }
                }
                assertEquals(PIECE_NUMBER, pool.nextNeededPiece())

                val dispatch = pool.scheduleAndDispatch(
                    head = PIECE_NUMBER,
                    maxInFlightPerPeer = 1
                )
                assertEquals(EXPECTED_CHUNK_REQUESTS, dispatch.scheduledFrames)
                assertEquals(EXPECTED_CHUNK_REQUESTS, dispatch.selectedFrames)
                assertEquals(EXPECTED_CHUNK_REQUESTS, dispatch.sentFrames)
                assertTrue(dispatch.failedPeerIds.isEmpty())
                assertEquals(PEER_ID, session.ownerOf(PIECE_NUMBER))

                assertTrue(peer.awaitChunkRequests())
                awaitCondition {
                    events.filterIsInstance<AceLiveTcpPoolEvent.Ingress>()
                        .flatMap { it.result.emittedPieces }
                        .any { it.piece == PIECE_NUMBER }
                }
                assertTrue(peer.awaitPieceResponsesAcknowledged())
                peer.throwIfFailed()

                val chunkIngress = events.filterIsInstance<AceLiveTcpPoolEvent.Ingress>()
                    .filter { it.result.activeChunkDispositions.isNotEmpty() }
                assertEquals(
                    listOf(
                        AceLiveChunkDisposition.ACCEPTED,
                        AceLiveChunkDisposition.ACCEPTED,
                        AceLiveChunkDisposition.PIECE_COMPLETED
                    ),
                    chunkIngress.flatMap { it.result.activeChunkDispositions }
                )
                assertEquals(
                    listOf(
                    ...
                    chunkIngress.flatMap { it.result.reassemblyDispositions }
                )

                val emitted = chunkIngress.flatMap { it.result.emittedPieces }
                assertEquals(1, emitted.size)
                val piece = emitted.single()
                assertEquals(PIECE_NUMBER, piece.piece)
                assertEquals(PEER_ID, piece.sourcePeerId)
                assertArrayEquals(PIECE_HEADER, piece.pieceHeader)
                assertArrayEquals(PIECE_DATA, piece.data)

                assertEquals(PIECE_NUMBER + 1, pool.nextNeededPiece())
                assertEquals(null, session.ownerOf(PIECE_NUMBER))
    ...
    private class DualLiveChunkPeer(
        ...
        private val pieceResponsesAcknowledged = CountDownLatch(1)
        ...
        private val tcpWorker = thread(
            start = true,
            isDaemon = true,
            name = "ace-session-reassembly-tcp-peer"
        ) {
            try {
        ...
        private val utpWorker = thread(
            start = true,
            isDaemon = true,
            name = "ace-session-reassembly-utp-peer"
        ) {
            try {
                ...
                val handshakeData = receiveUtpPacket()
                ...
                sendUtp(
                    AceLiveUtpPacket(
                        ...
                        payload = frame(
                            id = 11,
                            payload = compactLiveStatus(
                                minPiece = PIECE_NUMBER,
                                maxPiece = 12,
                                position = 12
                            )
                        ) + frame(id = 1)
                ...
                var latestClientSequence = interested.header.sequenceNumber
                repeat(EXPECTED_CHUNK_REQUESTS) { index ->
                    ...
                    latestClientSequence = chunkRequest.header.sequenceNumber
                    sendClientDataAck(
                        syn = receivedSyn,
                        acknowledgementNumber = latestClientSequence,
                ...
                repeat(EXPECTED_CHUNK_REQUESTS) { index ->
                    val serverSequence = UTP_SERVER_FIRST_CHUNK_SEQUENCE + index
                    sendUtp(
                        AceLiveUtpPacket(
                            header = AceLiveUtpHeader(
                                type = AceLiveUtpPacketType.DATA,
                                connectionId = receivedSyn.header.connectionId,
                                timestampMicros = 110L + index * 10L,
                                timestampDifferenceMicros = 10,
                                receiveWindowBytes = 64 * 1024L,
                                sequenceNumber = serverSequence,
                                acknowledgementNumber = latestClientSequence
                            ),
                            payload = liveChunkFrame(index)
                        )
                    )
                    val chunkAck = receiveUtpPacket()
                    check(chunkAck.header.type == AceLiveUtpPacketType.STATE)
                    check(chunkAck.header.connectionId == clientUtpSendConnectionId())
                    check(chunkAck.header.acknowledgementNumber == serverSequence)
                }
                pieceResponsesAcknowledged.countDown()
            } catch (error: Throwable) {
                fail(error)
            }
        }
        ...
        fun awaitPieceResponsesAcknowledged(): Boolean =
            pieceResponsesAcknowledged.await(3, TimeUnit.SECONDS)

        fun throwIfFailed() {
            failure.get()?.let { throw AssertionError("dual live chunk peer failed", it) }
        }
            ...
            sendUtp(
                AceLiveUtpPacket(
                    header = AceLiveUtpHeader(
                        ...
                        timestampDifferenceMicros = 10,
                        receiveWindowBytes = 64 * 1024L,
                        sequenceNumber = UTP_SERVER_FIRST_CHUNK_SEQUENCE,
    ...
    private companion object {
        ...
        const val CHUNK_LENGTH_BYTES = 4
        ...
        const val UTP_SERVER_FIRST_CHUNK_SEQUENCE = 503
        ...
        val PIECE_DATA = ByteArray(10) { index -> (0x10 + index).toByte() }
        ...
        fun liveChunkFrame(chunkIndex: Int): ByteArray {
            val start = chunkIndex * CHUNK_LENGTH_BYTES
            val end = minOf(start + CHUNK_LENGTH_BYTES, PIECE_DATA.size)
            val chunkData = PIECE_DATA.copyOfRange(start, end)
            val payload = ByteBuffer.allocate(4 + 4 + AceLivePieceHeaderCodec.HEADER_SIZE_BYTES + 2 + chunkData.size)
                ...
                .put(chunkData)
                .array()
            return frame(id = 7, payload = payload)
        }
