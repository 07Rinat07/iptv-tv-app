...
class AceLiveActivePeerCoordinatorTest {
    ...
    @Test
    fun missingChunksAreRetriedBeforeWholePieceTimeout() {
        val coordinator = AceLiveActivePeerCoordinator(
            geometry = AceLiveTransportGeometry(
                pieceLengthBytes = 12,
                chunkLengthBytes = 4,
            ...
            maxInFlightPerPeer = 1,
            maxOutstandingChunksPerPiece = 3,
            chunkRequestRetryMillis = 1_000
        )
        coordinator.onPeerEvent(window(peerId = 7, min = 10, max = 20, unchoked = true))
        ...
        assertEquals(
            AceLiveChunkDisposition.ACCEPTED,
            coordinator.onChunk(
                chunk(peer = 7, piece = 10, index = 0, header = header, size = 4),
                nextNeeded = 10
            ).disposition
        )

        assertTrue(
            coordinator.schedule(nextNeeded = 10, head = 10, nowMillis = 999).isEmpty()
        )
        assertEquals(
            listOf(1, 2),
            coordinator.schedule(nextNeeded = 10, head = 10, nowMillis = 1_000)
                .map { it.chunkIndex }
        )
    }

    @Test
    fun retryingMissingChunksPreservesOutstandingWindowAndThenRefillsNewWork() {
        val coordinator = AceLiveActivePeerCoordinator(
            geometry = AceLiveTransportGeometry(
                pieceLengthBytes = 16,
                chunkLengthBytes = 4,
            ...
            maxInFlightPerPeer = 1,
            maxOutstandingChunksPerPiece = 2,
            chunkRequestRetryMillis = 1_000
        )
        coordinator.onPeerEvent(window(peerId = 7, min = 10, max = 20, unchoked = true))
        ...
        assertEquals(
            listOf(2),
            coordinator.schedule(nextNeeded = 10, head = 10, nowMillis = 1_001)
                .map { it.chunkIndex }
        )
    }
