...
The following mappings are installed on the Android video surface. In fullscreen mode that surface is expected to own focus. Outside fullscreen they apply only while the video surface has focus; when focus is on a Compose rail, channel browser, dialog or panel, the focused Compose control or the system may receive these keys instead.
...
While the production Player is fullscreen:
...
Outside fullscreen, the arrow keys are not converted into Player channel/volume actions by the Player key policy; normal Compose focus navigation remains available.

### Player mouse/touchpad gestures

The current Player input adapter has a specialized scroll mapping in addition to clickable controls. Regression coverage protects channel and volume actions generated by that adapter. List surfaces inside the Player (channel browser, channel drawer, panels) remain normal scrollable lists and must not inherit accidental channel/volume changes from list scrolling.
...
A navigation-related PR is complete only when the relevant unit/regression checks and the full exact-head Android CI are green. Player architecture/input changes additionally require the Player Refactor Guard. P2P/runtime changes have separate gates and, where required, real-device evidence; building an instrumentation APK is not a substitute for hardware validation.
