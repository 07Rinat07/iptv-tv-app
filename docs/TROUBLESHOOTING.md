...
Archive support is enabled only from explicit persisted catch-up capability that passes the fail-closed resolver. A normal live URL is not evidence that archive playback exists.

If an archive action is unavailable, verify whether the source actually declares supported catch-up metadata. Unknown provider-specific modes, invalid ranges, malformed templates and unsupported placeholders must remain disabled rather than guessed.
...
For a reproducible P2P failure, preserve the correlated runtime fields when available (`startup_id`, `runtime_id`, generation/path/channel) and the producer-stage progression used by the field gate:

`scheduled -> selected -> sent -> chunk_ingress -> accepted/rejected -> piece_completed -> authenticated/auth_rejected -> ts_resync_output -> media_appended`
...
See `docs/REMOTE_AND_MOUSE.md` when that guide is present in the same or a later release.
...
A troubleshooting finding is not complete merely because one manual retry succeeds. A production fix must follow the repository workflow: fresh `main`, a narrow temporary branch, focused regression coverage where appropriate, full exact-head Android CI, required specialized guards, review resolution, and merge only when the exact PR head is fully green and mergeable.
