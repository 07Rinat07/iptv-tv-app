...
5. `Multiview`: базовые режимы `2-up` и `4-up` уже доступны; дальше нужны полноценная проверка производительности устройства перед запуском нескольких потоков и более быстрый UX выбора каналов для каждого окна.
