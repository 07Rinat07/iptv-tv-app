...
2. `Provider Accounts`: сохранение, проверка статуса и синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend` уже доступны в MVP; секреты шифруются через Android Keystore; дальше нужны адаптеры `Jellyfin/Plex`.
