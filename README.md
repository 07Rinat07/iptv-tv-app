...
5. `Multiview`: базовые режимы `2-up` и `4-up`, ограничение 4-up по runtime capability и быстрый выбор канала для целевого окна `2/3/4` уже доступны; дальше нужна полноценная проверка производительности устройства перед запуском нескольких потоков.
