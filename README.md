...
- Исправлена ручная остановка в фазе `Сохранение найденного` (остановка применяется без длительного зависания).
- Ускорен импорт найденного: пакетная обработка, параллельные импорты и таймаут на один импорт.
- Расширен web fallback: добавлен поиск и импорт прямых `m3u/m3u8` ссылок из открытой выдачи.
...
- Плеер:
  ...
  - быстрый запуск из списка: `Играть` сразу переключает выбранный канал и запускает встроенный плеер;
...
- Фиксированная копия этой же сборки:
  - `build-artifacts/myscanerIPTV-tvbox-debug-20260221-193859.apk`
...
10. Откройте `Плеер`, выберите канал и тип плеера (встроенный/VLC).
