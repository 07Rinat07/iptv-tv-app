...
- Рекомендуемые артефакты для проверки:
  - `build-artifacts/myscanerIPTV-tvbox-debug-latest.apk` (актуальная debug-сборка);
  - `build-artifacts/myscanerIPTV-tvbox-debug-YYYYMMDD-HHMMSS.apk` (сборка с таймстампом).
...
- Экспорт TXT:
  ...
  - затем сохранение через `Сохранить как...` или `Сохранить в память ТВ`.
- Экспорт логов:
