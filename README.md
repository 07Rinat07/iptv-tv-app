...
2. `Provider Accounts`: сохранение, проверка статуса, ручная и периодическая синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; структурированная диагностика `auth/network/parser/empty playlist` и краткая sync-history по каждому провайдеру уже показываются в UI, дальше нужен ещё более удобный UX сохранённых аккаунтов.
