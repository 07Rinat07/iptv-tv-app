...
2. `Provider Accounts`: сохранение, проверка статуса, ручная и периодическая синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; дальше нужны улучшения UX, история sync по каждому провайдеру и более глубокая диагностика в UI.
