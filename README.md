...
- Сканер публичных M3U/M3U8:
  ...
  - при нажатии `Остановить` во время сохранения импорт не обрывается (остановка откладывается до завершения сохранения);
...
- Плеер:
  ...
  - защита от спама повторяющихся `player_epg_error` в логах.
...
- Фиксированная копия этой же сборки:
  - `build-artifacts/myscanerIPTV-tvbox-debug-20260221-120154.apk`
...
- Экспорт TXT:
  ...
  - сохраняется в каталог загрузок приложения (`Android/data/.../files/Download`) или через `Сохранить как...`.
- Экспорт логов:
