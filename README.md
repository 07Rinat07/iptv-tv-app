...
- Добавлена базовая очередь `Записи эфира`:
  ...
  - обычные `HTTP/HTTPS`, базовые `HLS .m3u8` и локальные файлы теперь реально записываются в app-specific папку `Downloads`, успешные/ошибочные записи логируются как `download_file_saved` / `download_file_error`;
...
- Загрузки torrent-задач пока базовые: очередь уже различает `magnet/acestream/.torrent/HLS/HTTP/local`, реально пишет `HTTP/HLS/local` артефакты в app-specific `Downloads`, резолвит torrent/Ace через Engine Stream и пишет типы/ошибки в диагностику, включая отдельные tracker/peer ошибки, но полноценный torrent-клиент с записью файлов ещё не встроен.
