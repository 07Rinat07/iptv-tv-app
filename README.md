...
- Добавлен пресет `Казахские каналы` для поиска казахстанских/казахскоязычных M3U/M3U8 (`KZ`, `Kazakhstan`, `қазақша`, `Қазақстан`, `iptv-org`, GitHub/raw).
- Найденные сканером плейлист-ссылки теперь можно экспортировать не только в TXT-отчет, но и в отдельный UTF-8 M3U8-плейлист.
- Глобальные ошибки приложения (`app_uncaught_exception`, `app_background_coroutine_error`, `app_low_memory`, `app_trim_memory`) пишутся в экспортируемые диагностические логи с контекстом устройства/памяти и маскировкой секретов.
...
- Сканер публичных M3U/M3U8:
  ...
  - пресеты поиска (общий, русские, казахские, мир, спорт, фильмы, Voxlist/Repo);
  ...
  - экспорт найденных в сканере ссылок в `TXT` (имя списка + URL + источник) и `M3U8` (UTF-8 плейлист ссылок);
  - автоэкспорт найденных ссылок в TXT и M3U8 после `автосохранения` и после `ручной остановки с сохранением`;
  - формат имени TXT: `Tv_list_<поисковый_запрос>_YYYYMMDD_HHMMSS.txt`;
  - формат имени M3U8: `Tv_list_<поисковый_запрос>_YYYYMMDD_HHMMSS.m3u8`;
...
- Диагностика и логи:
  ...
  - запись необработанных падений приложения, ошибок фоновых корутин и сигналов нехватки памяти в диагностические логи;
  - маскировка чувствительных значений (`password/token/mac/login/user`) в файловом и экспортируемом логах.
...
- SHA256:
  - `80724906246c72f092fef7cc71aae571031cf05d1d20ed3b53dca6162d94a43d`
...
4. Для готовых пресетов нажмите `Импортировать`; для поиска выберите пресет сканера (например, `Русские каналы` или `Казахские каналы`) и нажмите `Найти и сохранить найденное`.
...
- Экспорт найденных ссылок сканера в M3U8:
  - кнопка `Экспорт найденных ссылок (.m3u8)` сохраняет найденные плейлист-ссылки как UTF-8 M3U8;
  - после `Найти и сохранить найденное` M3U8 создаётся автоматически рядом с TXT;
  - формат имени: `Tv_list_<поисковый_запрос>_YYYYMMDD_HHMMSS.m3u8`;
  - это список ссылок на найденные плейлисты, а не развёрнутый общий список всех каналов внутри каждого найденного плейлиста.
- Экспорт логов:
...
- Ошибки уровня приложения пишутся как `app_uncaught_exception`, `app_background_coroutine_error`, `app_low_memory`, `app_trim_memory`.
