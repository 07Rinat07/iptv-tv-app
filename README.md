...
- EPG grid уже доступен, но ещё без общей вертикальной виртуализации больших XMLTV и без детального modal-просмотра передачи.
...
2. `Provider Accounts`: сохранение и синхронизация `Xtream Codes`/`Stalker Portal` уже доступны в MVP; секреты шифруются через Android Keystore; дальше нужны адаптеры `Jellyfin/Plex/Tvheadend/HdHomeRun` и проверка статуса аккаунта.
