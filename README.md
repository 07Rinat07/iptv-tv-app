...
2. `Provider Accounts`: сохранение, проверка статуса и синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; дальше нужны улучшения UX, автообновления и расширенная диагностика провайдеров.
