...
2. `Provider Accounts`: сохранение, проверка статуса, ручная и периодическая синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; структурированная диагностика `auth/network/parser/empty playlist` уже показывается в UI, дальше нужны история sync по каждому провайдеру и ещё более удобный UX сохранённых аккаунтов.
