...
- Добавлен первый экран `Телепрограмма`:
  ...
  - для больших XMLTV EPG-строки показываются порциями с кнопкой `Показать ещё`, чтобы не перегружать экран сотнями каналов сразу.
...
- EPG grid и detail modal передачи уже доступны; XMLTV индексируется по каналам/времени, но общая вертикальная виртуализация UI для очень больших EPG ещё требует отдельной полировки.

## Roadmap расширенных TV-функций
1. `EPG Guide`: grid-экран с временной шкалой, фильтрами, поиском, detail modal, переходом в плеер, записью по EPG и внутренней индексацией XMLTV уже доступен; дальше нужна общая вертикальная виртуализация UI и стресс-проверка огромных XMLTV.
