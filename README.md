...
- Каталог логотипов уже вынесен в локальный asset logo pack, parser готов к внешним JSON pack-ам, а редактор умеет применять внешний JSON pack к текущему плейлисту; сетевой cache и полноценный ручной менеджер метаданных ещё в работе.
...
8. `Logo/Metadata Resolver`: общий resolver, локальный asset logo pack, parser/применение внешних JSON logo pack-ов из редактора и подбор по `tvg-id`, имени, стране, языку и домену источника уже доступны; дальше нужны сетевой cache и полноценный ручной менеджер метаданных.
