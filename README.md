...
- **Системные aggregate-списки**: `Все каналы` объединяет исходные плейлисты, а `Недавние` показывает последние доступные просмотренные каналы без повторов. Оба списка обновляются автоматически, не создают физические Room-строки и сохраняют конкретный playback context канала.
...
7. системный список **«Недавние»** автоматически собирает последние доступные каналы из истории просмотра; очистка истории очищает и этот список.
...
## Избранное и виртуальные списки
...
`Все каналы` и `Недавние` используют те же non-destructive правила. `Все каналы` сохраняет concrete `playlistId/channelId` исходной строки, а `Недавние` строится в MRU-порядке из истории, исключает устаревшие/неразрешимые, скрытые и заблокированные parental policy записи и не дублирует один logical channel. Физические refresh/delete/editor/EPG-операции для этих aggregate-списков также недоступны.
...
Source reconciliation, preferred-source picker и его справка завершены PR #177–#179. Системные aggregate-списки `Все каналы` и `Недавние`, catalog focus hot path и off-Main latest-wins rebuild завершены до production `2d22aea`. Разработка остановлена по запросу пользователя; aggregate collector/summary hardening остаётся следующим этапом после возобновления. Обычный M3U/M3U8 остаётся универсальным interoperability-экспортом и не заменяет полный backup приложения.
