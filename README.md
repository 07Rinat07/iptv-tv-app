...
- Добавлена базовая очередь `Записи эфира`:
  ...
  - добавлен `RecordingQueueWorker`, который каждые 15 минут проверяет due-задачи и сохраняет прямые `HTTP/HTTPS` потоки во внутреннюю папку приложения `files/recordings`;
  ...
  - можно удалить запись вместе с внутренним файлом;
...
- Запись эфира пока поддерживает только прямые `HTTP/HTTPS` потоки во внутреннюю папку приложения; внешняя папка записей, ffmpeg/Media3-рекордер, timeshift-буфер и HLS segment merge ещё не финализированы.
...
3. `Recording/Timeshift`: запись текущего эфира, запись по EPG, расписания, включение/выключение расписаний, папка записей, статусы и автоочистка старых файлов.
