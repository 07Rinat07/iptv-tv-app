...
- Каталог логотипов уже вынесен в локальный asset logo pack, parser готов к внешним JSON pack-ам, а редактор умеет применять внешний JSON pack к текущему плейлисту и вручную править metadata override для страны/языка/категории; сетевой cache и более массовый metadata manager ещё в работе.
...
4. `Player Tracks`: UI выбора аудиодорожки, субтитров и видеотрека на базе Media3 уже доступен во встроенном плеере; дальше можно полировать TV-UX и сохранение предпочитаемых дорожек.
...
6. `Android TV Home`: публикация рядов `Недавние каналы`, `Избранное`, `Продолжить просмотр`, `Записи` через `TvProvider`, ручной publish, периодический worker, очистка отключённых/пустых рядов и диагностика уже доступны; дальше нужна полировка карточек, восстановление после удаления системных каналов и проверка на реальных TV Box.
...
8. `Logo/Metadata Resolver`: общий resolver, локальный asset logo pack, parser/применение внешних JSON logo pack-ов из редактора, ручные override-ы логотипа/страны/языка/категории и подбор по `tvg-id`, имени, стране, языку и домену источника уже доступны; дальше нужны сетевой cache и более массовый metadata manager.
9. `Torrent/загрузки`: очередь, статусы, progress, пауза/возобновление/отмена, классификация источников `magnet/acestream/.torrent/HLS/HTTP/local` и engine-aware resolve для torrent/Ace задач уже доступны; дальше нужен реальный engine downloader с записью сегментов/файлов, проверкой места и ошибками по трекерам/пирам.
