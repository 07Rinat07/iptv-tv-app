...
- Добавлен первый рабочий контур `Provider Accounts`:
  ...
  - в карточке провайдера теперь видна история последних sync-операций по этому `providerId`: тип провайдера, playlistId, причина ошибки и detail берутся из diagnostic logs и показываются в раскрытых деталях;
...
2. `Provider Accounts`: сохранение, проверка статуса, ручная и периодическая синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; структурированная диагностика `auth/network/parser/empty playlist`, sync-history с type/playlistId/reason/detail, фильтрация/сворачивание аккаунтов и настраиваемый авто-sync `6/12/24h` уже есть в UI, дальше можно развивать ещё более богатый management UX и отдельную persisted sync-history таблицу при необходимости.
