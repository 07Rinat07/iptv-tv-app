...
- Добавлен первый рабочий контур `Provider Accounts`:
  ...
  - в карточке провайдера теперь видна краткая история последних sync-операций по этому `providerId` на основе диагностических логов;
...
- Загрузки torrent-задач пока базовые: очередь уже различает `magnet/acestream/.torrent/HLS/HTTP/local` и пишет типы в диагностику, но полноценный torrent-клиент/движок загрузки ещё не встроен.
...
2. `Provider Accounts`: сохранение, проверка статуса, ручная и периодическая синхронизация `M3U URL`/`Xtream Codes`/`Stalker Portal`/`HDHomeRun`/`Tvheadend`/`Jellyfin`/`Plex` уже доступны в MVP; структурированная диагностика `auth/network/parser/empty playlist`, краткая sync-history, фильтрация/сворачивание аккаунтов и настраиваемый авто-sync `6/12/24h` уже есть в UI, дальше можно развивать ещё более богатый management UX и отдельную историю sync по каждому провайдеру.
...
9. `Torrent/загрузки`: очередь, статусы, progress, пауза/возобновление/отмена и классификация источников `magnet/acestream/.torrent/HLS/HTTP/local` уже доступны; дальше нужен реальный torrent/engine worker с загрузкой сегментов/файлов, проверкой места и ошибками по трекерам/пирам.
